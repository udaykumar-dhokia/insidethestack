...
export function FloatingActionButtons({ articleId, title, platformUrl }: FloatingActionButtonsProps) {
  ...
  return (
    <div className="fixed bottom-6 right-6 md:bottom-10 md:right-10 flex flex-col gap-3 z-50">
      ...
      {/* X / Twitter Share */}
      <a
        href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(`Check out this deep dive on ${title} by @udaykumardhokia`)}&url=${encodeURIComponent(url)}`}
        target="_blank"
        rel="noopener noreferrer"
        className={baseButtonClass}
        title="Share on X"
      ...
      >
        <svg className="w-6 h-6" viewBox="0 0 24 24" fill="currentColor">
          <path d="M2 2h20v20H2z" fill="#ff6600"/>
          <path d="M12.39 12.333L16 5h-2.11L11.39 10.333 8.89 5H6.77l3.61 7.333V18h1.83v-5.667z" fill="white"/>
        </svg>
      </a>
